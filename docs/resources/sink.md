...
subcategory: ""
description: |-
  Manages Pulsar IO sinks through the Functions Worker API.
---
...
- `parallelism` (Number) The sink's parallelism factor. Defaults to `1`.
...
- `receiver_queue_size` (Number) The consumer receiver queue size for this topic. Pulsar defaults to 1000 when unset, which buffers up to that many messages per sink instance.
...
```shell
terraform import pulsar_sink.example public/default/example
```

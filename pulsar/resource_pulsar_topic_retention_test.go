...
// from topic configuration removes topic-level retention policy in Pulsar.
func TestAccPulsarTopic_retentionPolicyRemoval(t *testing.T) {
	skipIfNoTopicPolicies(t)
	resourceName := "pulsar_topic.test"
	topicName := "retention-removal-" + acctest.RandString(10)
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testTopicWithRetention(testWebServiceURL, topicName, 1600, 20000),
			...
			{
				Config: testTopicWithoutRetention(testWebServiceURL, topicName),
				Check: resource.ComposeTestCheckFunc(
					testPulsarTopicExists(resourceName, t),
...
func testTopicWithRetention(url, topicName string, retentionMinutes, retentionSizeMB int) string {
	return fmt.Sprintf(`
provider "pulsar" {
  web_service_url = "%s"
}
...
`, url, topicName, retentionMinutes, retentionSizeMB)
}

func testTopicWithoutRetention(url, topicName string) string {
	return fmt.Sprintf(`
provider "pulsar" {
  web_service_url = "%s"
}
	...
	return func(s *terraform.State) error {
		...
		if !shouldExist {
			if err != nil {
				...
				return fmt.Errorf("ERROR_GETTING_TOPIC_RETENTION: %w", err)
			}
			...
			return fmt.Errorf(
				"expected topic retention to be removed, but got time=%d size=%d",
		...
		if ret == nil {
			return fmt.Errorf("expected topic retention policy to exist, but got nil")
		}

		if ret.RetentionTimeInMinutes != expectedMinutes {
			return fmt.Errorf("expected retention_time_minutes=%d, got=%d", expectedMinutes, ret.RetentionTimeInMinutes)
		}
		if ret.RetentionSizeInMB != expectedSizeMB {
			return fmt.Errorf("expected retention_size_mb=%d, got=%d", expectedSizeMB, ret.RetentionSizeInMB)
		}
